...
########## 1.6 survival plot for mutation ######
final_msg_score_gene_df <- read.csv("~/onedrive/Work/phD/phd_project/SiT/results/function/final_msg_score_gene_df.csv")
final_msg_score_gene_df <- final_msg_score_gene_df[final_msg_score_gene_df$Approved.symbol != "KRT8",]
mag_genes <-  final_msg_score_gene_df$Approved.symbol
...
mut_baiduI <- mut_baiduI[mut_baiduI$Hugo_Symbol %in% mag_genes, ]
mut_baiduII <- mut_baiduII[mut_baiduII$Hugo_Symbol %in% mag_genes, ]

altered_patients <- unique(c(mut_baiduI$Tumor_Sample_Barcode, mut_baiduII$Tumor_Sample_Barcode))
length(altered_patients)
length(unique(mut_baiduI$Tumor_Sample_Barcode))
length(unique(mut_baiduII$Tumor_Sample_Barcode))
...
metadata_baiduI_survival$MAG_mutated <- ifelse(metadata_baiduI_survival$Sample_ID %in% altered_patients, "MSG_mutated", "MSG_unmutated")
...
metadata_baiduII_survival$MAG_mutated <- ifelse(metadata_baiduII_survival$Sample_ID %in% altered_patients, "MSG_mutated", "MSG_unmutated")
...
metadata_baiduI_baiduII_survival$MAG_mutated <- ifelse(metadata_baiduI_baiduII_survival$Sample_ID %in% altered_patients, "MSG_mutated", "MSG_unmutated")

table(metadata_baiduI_baiduII_survival$MAG_mutated)

#  MSG_mutated MSG_unmutated 
#  542           113 

### survival plot
km_fit_mag <- survfit(Surv(Survival_time, Survival_status) ~ MAG_mutated, data= metadata_baiduI_baiduII_survival)
summary(km_fit_mag)
ggsurvplot(km_fit_mag, data = metadata_baiduI_baiduII_survival,conf.int = T, pval = T)

km_baiduI_fit_mag <- survfit(Surv(Survival_time, Survival_status) ~ MAG_mutated, data= metadata_baiduI_survival)
summary(km_baiduI_fit_mag)
ggsurvplot(km_baiduI_fit_mag, data = metadata_baiduI_survival,conf.int = T, pval = T)

km_baiduII_fit_mag <- survfit(Surv(Survival_time, Survival_status) ~ MAG_mutated, data= metadata_baiduII_survival)
summary(km_baiduII_fit_mag)
ggsurvplot(km_baiduII_fit_mag, data = metadata_baiduII_survival,conf.int = T, pval = T)
...
###### 2. TCGA data ######
count <- read.csv("rawdata/TCGA_80cases_counts.csv",header = T, row.names = 1)
colnames(count) <- gsub("\\.", "-",colnames(count))
rownames(count) <- sapply(strsplit(rownames(count),"\\."),"[[",1)

metadata <- read.csv(file = "rawdata/TCGA_80cases_clinical_data.csv", row.names = 1,header = TRUE)
...
##### survival plot ####
gsva.res <- gsva(as.matrix(count), list(final_msg_score_gene_df$Ensembl.gene.ID), method = "gsva")
gsva.res  <- as.data.frame(t(gsva.res ))
summary(gsva.res )
gsva.res$mag_score_group <- ifelse(gsva.res[,1] >= -0, "High","Low")
gsva.res$Sample_ID <- rownames(gsva.res)
table(gsva.res$mag_score_group)
...
km_tcga_MSG_score_fit <- survfit(Surv(Survival_time, Survival_status) ~ mag_score_group, data=metadata_survival)

ggsurvplot(km_tcga_MSG_score_fit, linetype = "strata", break.time.by = 250, 
           palette = c("#E7B800", "#2E9FDF"), conf.int = T, pval = T, data = metadata_survival, risk.table = T)

ggsave("~/onedrive/Work/phD/phd_project/SiT/results/function/TCGA_down_MSG_scores_survival.pdf")
#### 不太行。。。
